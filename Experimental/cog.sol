    ...
    DividendPayingTokenOptionalInterface
{
    ...
    function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            IERC20(WCRO).transfer(user, _withdrawableDividend);
...
contract DividendTracker is Ownable, DividendPayingToken {
    ...
    constructor(address _wcro)
        public
        DividendPayingToken("DividendTracker", "DividendTracker", _wcro)
    {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 100 * (10**9); //must hold 350M+ tokens
    }
    ...
    function updateMinimumTokenBalanceForDividends(uint256 newMinimumTokenBalanceForDividends) external onlyOwner {
        ...
        uint256 _oldMinimumTokenBalanceForDividends = minimumTokenBalanceForDividends;
        require(
            _oldMinimumTokenBalanceForDividends != newMinimumTokenBalanceForDividends,
            "same value"
        );
        emit MinimumTokenBalanceForDividendsUpdated(newMinimumTokenBalanceForDividends, _oldMinimumTokenBalanceForDividends);
...
// File contracts/CronosOnlyGemsToken.sol
...
contract CronosOnlyGems is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public croRewardsFee = 6;
    uint256 public liquidityFee = 1;
    uint256 public marketingFee = 1;
    uint256 public totalFees =
        croRewardsFee.add(liquidityFee).add(marketingFee);
    ...
    event MaxWalletBalanceUpdated(uint256 percentage);
    ...
    ) public ERC20(name, symbol,9) {
        wcro = _uniswapV2Router.WETH();
    ...
    function initialize(address dividendTrackerAddress) external onlyOwner {
        ...
        isTxLimitExempt[_uniswapV2Pair] = true;
        ...
        */
        _mint(_owner, 100_000_000 * (10**9));
        maxTransactionAmount =   200_000 * (10 ** 9);
        maxWalletBalance = totalSupply() * 10 / 1000;
    }
    ...
    function setFees(
        uint256 _croRewardsFee,
        uint256 _liquidityFee,
        uint256 _marketingFee
    ) external onlyOwner {
        require(
            _croRewardsFee + _liquidityFee + _marketingFee <= 20,
            "COG: Too high fees"
        );
        ...
        totalFees = _croRewardsFee.add(_liquidityFee).add(_marketingFee);

        emit FeesChanged(_croRewardsFee, _liquidityFee, _marketingFee);
    }
    ...
    function _transfer(
        ...
        require(amount <= maxTransactionAmount || isTxLimitExempt[from], "TX Limit Exceeded");
        ...
        if(!excludedFromMaxBalance[to] && restrictWhales) {
    ...
    function swapAndSendToFee(uint256 tokens, address dest) private {
        address _wcro = wcro;
        uint256 initialCROBalance = address(this).balance;

        swapTokensForEth(tokens);

        uint256 dividends = address(this).balance.sub(initialCROBalance);
        IWETH(_wcro).deposit{value: dividends}();
        IERC20(_wcro).transfer(dest, dividends);
    }
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        try _dividendTracker.distributeDividends(dividends) {
            ...
            IERC20(_wcro).transfer(address(_dividendTracker), dividends);
